...
function App() {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = `http://127.0.0.1:5223/api/send?nodeUrl=${nodeUrl}&peerUrl=${peerUrl}&subscription=${subscription}&message=${message}`;
  ...
  const handleApiCall = async (event) => {
    event.preventDefault();
    const apiUrl = `http://127.0.0.1:5223/api/see`;
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            <input
              ...
              type="text"
              value={peerUrl}
              placeholder='Peer to connect to'
              className="login-input"
